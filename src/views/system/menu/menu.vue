      ...
      <n-gi span="1">
        <n-card :segmented="{ content: true }" :bordered="false" size="small">
          ...
          <div class="w-full menu">
            <n-input type="text" v-model:value="pattern" placeholder="Enter menu name to search">
              <template #suffix>
      ...
      <n-gi span="2">
        <n-card :segmented="{ content: true }" :bordered="false" size="small">
          ...
          <n-alert type="info" closable>
            After selecting an item from the menu list, edit it</n-alert
          >
          <n-form
            :model="formParams"
            :rules="rules"
...
<script lang="ts" setup>
  ...
  function selectAddMenu(key: string) {
    drawerTitle.value = key === 'home' ? 'Add top bar menu' : `Add submenu: ${treeItemTitle.value}`;
    openCreateDrawer();
  }
  ...
  function formSubmit() {
    formRef.value.validate((errors: boolean) => {
      if (!errors) {
        message.error('Sorry, you do not have this permission');
      } else {
